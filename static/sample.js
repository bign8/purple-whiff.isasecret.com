...
// Other: http://kennethjorgensen.com/blog/2014/script/canvastree.js

var MeruLogo = (function(window) {
	var INTRO_STATE = 0;
	var MORPH_STATE = 1;
	var OUTRO_STATE = 2;
	...
	var options = {
		containerID: "meru-logo",
		logoCanvasID: "meru-logo-canvas",
		shadowCanvasID: null,
		wordImageURL: "http://meru.ca/wp-content/themes/merutheme/images/meru-word.png",
		wordOffsetX: -25,
		wordOffsetY: 0,
		wordScale: 1,
	...
	API.initialize = function(overrideOptions) {
		...
		MeruLogo.util.extendObject(options, overrideOptions);
	...
	API.applyImpulse = function(x, y, strength, radius) {
		function applyForceToShape(shape) {
			...
			for (var i = 0, len = shape.points.length; i < len; i++) {
				...
				var distance = MeruLogo.util.distanceBetween(target, p);
				if (distance < radius) {
	...
	function enableDebugging() {
		...
		var configureScript = "MeruLogo.initialize({\n";
		...
		for (var i = 0; i < options.colors.length; i++) {
			...
			configureScript += "\t\t\t [ new MeruLogo.util.Color( " + ca.r + ", " + ca.g + ", " + ca.r + " ), \t new MeruLogo.util.Color( " + ca.r + ", " + ca.g + ", " + ca.r + " ) ],\n"
		}
		configureScript += "\t\t ]\n";
		configureScript += "});";
	...
	function updateShape(shape) {
		...
		for (var i = 0, len = shape.points.length; i < len; i++) {
			p1 = MeruLogo.util.getLoopedArrayElement(shape.points, i);
			...
			if (MeruLogo.util.distanceBetween(p1, p1.normal) > MORPH_RANGE2) {
	...
	function renderLogo() {
		...
		renderShape(shapeBackground, new MeruLogo.util.Color(0, 0, 0, 0.5));
	...
	function renderShape(shape, color) {
		...
		var p1 = MeruLogo.util.getLoopedArrayElement(shape.points, -1);
		var p2 = MeruLogo.util.getLoopedArrayElement(shape.points, 0);
		logoContext.moveTo(p1.x + (p2.x - p1.x) / 2, p1.y + (p2.y - p1.y) / 2);
		for (var i = 0, len = shape.points.length; i <= len; i++) {
			p1 = MeruLogo.util.getLoopedArrayElement(shape.points, i);
			p2 = MeruLogo.util.getLoopedArrayElement(shape.points, i + 1);
...
*
*
MeruLogo.input.move = (function( window ) {
...
//
MeruLogo.applyForce( local.x, local.y, force.x * FORCE_STRENGTH, force.y * FORCE_STRENGTH, FORCE_RADIUS );
...
MeruLogo.applyParallax( fx, fy, bx, by );
...
canvas = $( "#" + MeruLogo.getOptions().logoCanvasID );
}
}
...
*
*
MeruLogo.input.press = (function( window ) {
...
var scale = MeruLogo.getScale();

MeruLogo.applyImpulse( local.x, local.y, strength * scale, IMPULSE_RADIUS * scale );
